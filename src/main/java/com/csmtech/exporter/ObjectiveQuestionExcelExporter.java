...
public class ObjectiveQuestionExcelExporter {
	...
	private void writeHeaderLine() {
		 ...
		 * 
		 * // Merge cells for the title row sheet.addMergedRegion(new
		 * CellRangeAddress(0, 0, 0, 1));
		 * 
		 * // Create a cell style for the title CellStyle titleStyle =
		 * workbook.createCellStyle(); XSSFFont titleFont = workbook.createFont();
		 * titleFont.setBold(true); titleStyle.setFont(titleFont);
		 * titleStyle.setAlignment(HorizontalAlignment.CENTER);
		 * 
		 * // Apply the style to the title cell titleCell.setCellStyle(titleStyle);
		 */
